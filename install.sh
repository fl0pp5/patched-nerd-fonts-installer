...
General options:
  ...
  -p <target-path>              Path to install fonts
  -f <source-path>              Path to source with fonts
EOF
}
...
nerdfonts_root_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/$sourcepath"
...
sys_font_dir=$installpath
usr_font_dir=$installpath
font_dir=$installpath
